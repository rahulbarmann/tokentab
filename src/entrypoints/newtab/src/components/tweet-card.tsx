...
export function ContentCard({ card, isExpanded = false }: { card: ContentCardType; isExpanded?: boolean }) {
  if (card.type === 'tweet') {
    // Get the first image from HTML content if it exists
    const htmlImage = extractFirstImage(card.html)
    // Use either the media image or the first HTML image
    const imageToShow = card.media?.url || htmlImage

    // Remove img tags from HTML content since we'll show the image separately
    ...
    return (
      ...
      >
        ...
        <div className="text-[10px] text-gray-400">{new Date(card.timestamp).toLocaleDateString()}</div>
      </Card>
    )
  }
